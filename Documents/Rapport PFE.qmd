...
L'*ACAPS* propose une méthode de détermination des flux d'ajustement futurs décrite à **l'annexe N°6** de la circulaire de l'ACAPS. Cette méthode suit un certain nombre d'étapes que nous décrivons ci-après
...
$$
CSR_I = BEC_{eng}(Choc) - BE_{eng} \quad ,
$$
avec : 

$BE_{eng}(Choc)$ : La meilleure estimation des engagements choqué.
...
Tel que:  $$CSR^{j}_{i} = BE^{\text{scénario j}} - BE \qquad j \in \{ \text{augmentation} ; \text{diminution}\}$$  
...
$$
CSR_{Cp} = \sqrt{\sum_{i,j\in type}(\rho_{i,j}\times CSR_{Cp}^{i}\times CSR_{Cp}^{j})}
$$
...
##### Exigence de capital pour risque de mortalité ($CSR_{Mor}$)
...
Selon l'Acaps, le taux de choc pour la mortalité est estimé à 30 %.
...
d’abord c'est quoi un rachat en assurance vie ?
...
$$
CSR_{P} = \sqrt{\sum_{i,j} ( \rho_{i,j}* CSR_i * CSR_j)}
$$
avec : 
...
$$
Ajt_{ID} = TI \times min(CSRB + CSRO - Adj_{As} \quad ; \quad ID_{p} - ID_{a})
$${#eq-gbmp}
avec :
