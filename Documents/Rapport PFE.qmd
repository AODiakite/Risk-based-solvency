...
Dans ce qui suit, on va on va présente les méthodes qu'on a utilisées pour la valorisation des actions cotées à la bourse de Casablanca. La première méthode était le modèle classique de Black & Scholes.

Pour la deuxième méthode, on a choisi d'entraîner un module de réseaux de neurones récurrents, plus précisément le module LSTM, avec des données historiques afin de prédire les cours futurs.
...
Apres le nettoyage des donnes qu’on a collectées, nous avons présenté premièrement le cours historique de clôture « Dernier » en fonction de temps, afin de voir le développement de cours par le temps, aussi prendre une idée sur la variation à long terme et à court terme pour cet indice.

<div style="text-align:center">
![Le cours Historiques de Clôture pour l’indice MASI](Rapport-PFE_files/figure-html/Cloture_masi.png){#fig-piliers}
</div>

Une fois les données sont préparées, nous avons effectué une normalisation des valeurs numériques pour les mettre à l'échelle.
...
Ensuite, on a divisé les données en deux ensembles, une pour l’entraînement, et la deuxième pour le test. L'ensemble d'entraînement est utilisé pour entrainer et ajuster les poids du module LSTM, tandis que l'ensemble de test permet d'évaluer les performances finales du modèle sur des données inconnues.
Une fois qu’on a terminé ces étapes, les données sont utilisées pour entraîner notre module LSTM, qui va nous permettre de prédire des cours futures.
...
<div style="text-align:center">
![Courbes de cours entraîné et cours réel](Rapport-PFE_files/figure-html/entrainement.png){#fig-piliers}
</div>

<div style="text-align:center">
![Courbes de cours prédit et cours réel](Rapport-PFE_files/figure-html/predection.png){#fig-piliers}
</div>
...
$$
BEGP = \sum\limits_{t=0}^{t=N}\frac{(Dec_{t}-Enc_{t})}{(1+r_t)^t}
$$
Avec :
...
$$
Dec_{t} = K  \times  NbP_t \times q_{x+t} \times (1+r_t)^t
$$
...
$$
FG_t = NbC_t \times \overline{FGU_t}
$$
...
$$
\overline{FGU_t} = \frac{\frac{FG_{t-1}}{NbC_{t-1}}+\frac{FG_{t-2}}{NbC_{t-2}}+\frac{FG_{t-3}}{NbC_{t-3}}}{3}
$$
...
$$
BE_{eng}= BE_{eng}^{sinistres} + BE_{eng}^{primes}
$$

##### La meilleure estimation des engagements pour primes($_{nvhr}BE_{eng}^{primes}$):
...
$$
BE_{eng}^{primes} = \sum_{t \geq 1} \frac{\overline{FRFP_{t}}}{(1+ r_t)^t}- PFPA
$$
...
$$
\overline{FRFP_{t}} = TL * RSM * (PPNA + PFP)
$$

Où:

- $TL$: le taux de liquidation,il est estimé en fonction de la cadence de liquidation des engagements pour sinistres survenus.
- $RSM$: le ratio de sinistralité moyen telle que: 
$$ 
RSM_t= \frac{CU_{t} + CU_{t-1} + CU_{t-2}}{PA_{t} + PA_{t-1}+ PA_{t-2}}
$$
...
- $RC_t$ : les règlements cumulés 
- $RF_t$: les règlements futurs
- $PPNA$: La provision pour primes non acquises.
- $PFP$: Le montant des primes futures probabilisé.

##### La meilleure estimation des engagements pour sinistres nets ($BE_{eng}^{sinistres}$):
...
$$ 
BE_{eng}^{sinistres} = \sum_{t \geq 1} \frac{FRFP_{t}}{(1+ r_t)^t} 
$$
avec:
...
$$
BEFG= \sum_{t \geq 1} \frac{FFGF_t}{(1+r_t)^t}
$$
...
$$
FFGF = TFGM * (BE_{eng}^{sinistres} + BE_{eng}^{primes})
$$
...
$$
TFGM= \frac{TFG_{t} + TFG_{t-1} + TFG_{t-2}}{3}
$$

ainsi que :

$$
TFG_{t} = \frac{FG}{BE_{eng}^{sinistres}+ REC}
$$
...
$$
BE_{eng}^{cedes} =\; BE_{sinsitres}^{cedes} + \; BE_{primes}^{cedes}
$$
Avec :

> $BE_{sinsitres}^{cedes} = TC_{sinsitres} \times BE_{eng}^{sinsitres}$ : La meilleure estimation des engagements pour sinistres cédés visée à la spécification technique 33 est évaluée en multipliant la meilleure estimation des engagements pour sinistres nets de recours par ($TC_{sinsitres}$) le rapport entre la part des cessionnaires dans les provisions pour sinistres à payer et la provision pour sinistres à payer brute de réassurance.

> $BE_{primes}^{cedes} = TC_{primes} \times BE_{eng}^{primes}$ : La meilleure estimation des engagements pour primes cédés est évaluée en multipliant la meilleure estimation des engagements pour primes par le taux de cession de primes. Le taux de cession ($TC_{primes}$) de primes précité correspond au rapport entre les primes brutes non vie (cessions) et les primes émises de l'exercice.
...
$$
BE_{eng\;i}^{\;cédès} = (BE_{eng\;i}^{sinsitres} \times RS_{0} + BE_{eng \;i}^{primes} \times RP_{0})
$$

Où:

- $BE_{eng\;i}^{sinsitres} = \sum\limits_{t>i}\frac{F_t}{(1+r_t)^t}$, avec $(r_t)$ calculés à partir de la courbe des taux fixée par l'Autorité.
- $BE_{eng \;i}^{primes}$: correspond à la meilleure estimation des engagements pour primes projetée à la date i.
...
> Pour les opérations d’assurance vie, décès ou de capitalisation
 ...
 $$
 BE_{eng\;i}^{\;cédès} = (MEGP_i + BDF_i)\times R_0
 $$
...
$$
 BE_{eng\;i}^{cédès} = MEGP_i\times R_0
$$
...
$$
Adj_i = \frac{1}{2}\times \max( BE_{eng\;i}^{cédès} - DEV + SDR;0)\times PD \times (1-PD)^{i-1}
$$
avec: 
...
$$
Adj_D = \sum\limits_{i>0} \frac{Adj_i}{(1+r_i)^i}
$$
...
$$
CSRB = \sqrt{\sum_{i,j\in Risk }(\rho_{i,j} * CSR_{i} * CSR_{j})}
$$
avec : 
...
##### L'exigence de capital relative au risque d'écart de taux ($CSR_{Ecart\;taux}$)
